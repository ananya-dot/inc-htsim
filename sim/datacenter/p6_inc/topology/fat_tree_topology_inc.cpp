...
#include "fat_tree_topology_inc.h"
#include <vector>
#include "string.h"
#include <sstream>
...
void FatTreeTopology::set_custom_params(uint32_t no_of_nodes) {
    ...
    // SUPERNODE: NSRV is the number of regular servers (without supernode)
    // The supernode will have node ID = NSRV
    // For custom topologies, we assume no_of_nodes includes the supernode if:
    // - It's a 3-tier topology
    // - (no_of_nodes - 1) is divisible by hosts_per_pod (meaning we have K^3/4 regular + 1 supernode)
    // Otherwise, all nodes are regular servers
    bool has_supernode = false;
    if (_tiers == 3 && no_of_nodes > 0 && (no_of_nodes - 1) % _hosts_per_pod == 0) {
        NSRV = no_of_nodes - 1;  // Regular servers (excluding supernode)
        has_supernode = true;
        cout << "SUPERNODE: Detected " << no_of_nodes << " total nodes, " 
             << NSRV << " regular servers, supernode ID = " << NSRV << endl;
        // Add virtual pod for supernode: 1 extra pod with 1 ToR and K/2 Agg switches
        // Calculate K from the number of regular pods
        uint32_t K = no_of_pods;  // Regular pods = K
        NPOD = no_of_pods + 1;  // Add virtual pod
        NTOR = _tor_switches_per_pod * no_of_pods + 1;  // Add 1 ToR for supernode pod
        NAGG = _agg_switches_per_pod * no_of_pods + (K/2);  // Add K/2 Agg switches for supernode pod
        cout << "SUPERNODE: Creating virtual pod with 1 ToR and " << (K/2) << " Agg switches" << endl;
    } else {
        NSRV = no_of_nodes;  // No supernode, or 2-tier topology
        if (_tiers == 3) {
            cout << "SUPERNODE: No supernode detected, " << NSRV << " regular servers" << endl;
        }
        NPOD = no_of_pods;
...
void FatTreeTopology::set_params(uint32_t no_of_nodes) {
    ...
    int K = 0;
    if (_tiers == 3) {
        // Find the largest K such that K^3/4 <= no_of_nodes
        // This handles both regular (K^3/4) and supernode (K^3/4 + 1) cases
        while (true) {
            int next_K = K + 1;
            uint32_t next_nodes = next_K * next_K * next_K / 4;
            if (next_nodes > no_of_nodes) {
                // next_K would be too large, so current K is the right one
                break;
            }
            K = next_K;
            _no_of_nodes = next_nodes;
        }
        
        // SUPERNODE: Allow either exactly K^3/4 nodes (no supernode) or K^3/4 + 1 (with supernode)
        if (no_of_nodes != _no_of_nodes && no_of_nodes != _no_of_nodes + 1) {
            cerr << "Topology Error: can't have a 3-Tier FatTree with " << no_of_nodes
                 << " nodes\n";
            cerr << "Valid node counts: " << _no_of_nodes << " (regular) or " 
                 << (_no_of_nodes + 1) << " (with supernode)\n";
            exit(1);
        }
        int NK = (K*K/2);
        NSRV = (K*K*K/4);  // Regular servers (without supernode)
        
        // Set _agg_switches_per_pod and _tor_switches_per_pod BEFORE using them in calculations
        _agg_switches_per_pod = K/2;
        _tor_switches_per_pod = K/2;
        cout << "DEBUG: set_params: K=" << K << " _agg_switches_per_pod=" << _agg_switches_per_pod 
             << " _tor_switches_per_pod=" << _tor_switches_per_pod << endl;
        cout << "DEBUG: set_params: Member variable addresses: &_tor_switches_per_pod=" << &_tor_switches_per_pod 
             << " &_agg_switches_per_pod=" << &_agg_switches_per_pod << endl;
        cout << "DEBUG: set_params: After setting, tor_switches_per_pod()=" << tor_switches_per_pod() 
             << " agg_switches_per_pod()=" << agg_switches_per_pod() << endl;
        
        // SUPERNODE: Total nodes = NSRV (regular) + 1 (supernode) = K^3/4 + 1
        // The supernode will have node ID = NSRV
        // Update _no_of_nodes to match the actual node count (may include supernode)
        bool has_supernode = (no_of_nodes == NSRV + 1);
        if (has_supernode) {
            // Supernode is included - create virtual pod for it
            _no_of_nodes = no_of_nodes;  // Includes supernode
            // Add 1 extra pod for the supernode (with 1 ToR and K/2 Agg switches)
            NPOD = K + 1;  // Extra pod for supernode
            NTOR = NK + 1;  // Extra ToR for supernode pod
            NAGG = NK + (K/2);  // Extra K/2 Agg switches for supernode pod
            cout << "SUPERNODE: Creating virtual pod with 1 ToR and " << (K/2) << " Agg switches" << endl;
        } else {
            // No supernode
            _no_of_nodes = NSRV;  // Only regular servers
            NPOD = K;
            NTOR = NK;
            NAGG = NK;
        }
    ...
    assert(_hosts_per_pod == 0);
    // _tor_switches_per_pod and _agg_switches_per_pod already set above
...
void FatTreeTopology::alloc_vectors() {
    ...
    // SUPERNODE: Use _no_of_nodes instead of NSRV to include supernode
    pipes_nlp_ns.resize(NTOR, vector< vector<Pipe*> >(_no_of_nodes, vector<Pipe*>(_bundlesize[TOR_TIER])));
    queues_nlp_ns.resize(NTOR, vector< vector<BaseQueue*> >(_no_of_nodes, vector<BaseQueue*>(_bundlesize[TOR_TIER])));
    ...
    // SUPERNODE: Use _no_of_nodes instead of NSRV to include supernode
    pipes_ns_nlp.resize(_no_of_nodes, vector< vector<Pipe*> >(NTOR, vector<Pipe*>(_bundlesize[TOR_TIER])));
    queues_nlp_nup.resize(NTOR, vector< vector<BaseQueue*> >(NAGG, vector<BaseQueue*>(_bundlesize[AGG_TIER])));
    queues_ns_nlp.resize(_no_of_nodes, vector< vector<BaseQueue*> >(NTOR, vector<BaseQueue*>(_bundlesize[TOR_TIER])));
    
    // SUPERNODE: No longer needed - supernode is now in a virtual pod
    // Old direct core-supernode connections are removed
}
...
void FatTreeTopology::init_network(){
    ...
    // SUPERNODE: Allocate vectors for all nodes including supernode
    // _no_of_nodes includes the supernode if present
    for (uint32_t j=0;j<NTOR;j++) {
        for (uint32_t k=0;k<_no_of_nodes;k++) {  // Include supernode in allocation
    ...
    // SUPERNODE: Include supernode in connections if present
    for (uint32_t tor = 0; tor < NTOR; tor++) {
        ...
        for (uint32_t l = 0; l < link_bundles; l++) {
            ...
            // Skip if this server index is beyond the actual number of nodes
            if (srv >= _no_of_nodes) break;
            for (uint32_t b = 0; b < _bundlesize[TOR_TIER]; b++) {
                // Downlink
                if (_logger_factory) {
    ...
    // SUPERNODE: Create connections for the virtual pod's Agg switches to all core switches
    // The supernode pod (last pod) has K/2 Agg switches that connect to all core switches
    if (_tiers == 3 && _no_of_nodes == NSRV + 1) {
        uint32_t supernode_id = NSRV;
        uint32_t supernode_pod = NPOD - 1;  // Last pod is the supernode pod
        uint32_t supernode_tor = NTOR - 1;  // Last ToR is in the supernode pod
        // SUPERNODE: Calculate first Agg switch in supernode pod
        // Regular pods: 0 to NPOD-2, each with _agg_switches_per_pod Agg switches
        // Supernode pod: NPOD-1, with _agg_switches_per_pod Agg switches
        // So agg_start = (NPOD-1) * _agg_switches_per_pod
        uint32_t agg_start = (NPOD - 1) * _agg_switches_per_pod;  // First Agg switch in supernode pod
        
        cout << "SUPERNODE: Creating virtual pod connections" << endl;
        cout << "  Supernode ID: " << supernode_id << endl;
        cout << "  Supernode pod: " << supernode_pod << endl;
        cout << "  Supernode ToR: " << supernode_tor << endl;
        cout << "  Agg switches in supernode pod: " << agg_start << " to " << (agg_start + _agg_switches_per_pod - 1) << endl;
        
        // Connect supernode pod's Agg switches to all core switches
        for (uint32_t agg = agg_start; agg < agg_start + _agg_switches_per_pod; agg++) {
            uint32_t podpos = agg % _agg_switches_per_pod;
            for (uint32_t l = 0; l < _radix_up[AGG_TIER]/_bundlesize[CORE_TIER]; l++) {
                uint32_t core = podpos + _agg_switches_per_pod * l;
                assert(core < NCORE);
                ...
                for (uint32_t b = 0; b < _bundlesize[CORE_TIER]; b++) {
                    ...
                    assert(switches_c[core]->addPort(queues_nc_nup[core][agg][b]) < 64);
                    assert(switches_up[agg]->addPort(queues_nup_nc[agg][core][b]) < 64);  // FIX: Add queue to Agg switch
                    queues_nc_nup[core][agg][b]->setRemoteEndpoint(switches_up[agg]);  // FIX: Set remote endpoint for Core->Agg queue
                    queues_nup_nc[agg][core][b]->setRemoteEndpoint(switches_c[core]);
        ...
        cout << "SUPERNODE: Virtual pod connections created successfully" << endl;
    }
...
vector<const Route*>* FatTreeTopology::get_bidir_paths(uint32_t src, uint32_t dest, bool reverse){
    ...
    // SUPERNODE: Handle routing to/from the supernode
    // The supernode (node ID = NSRV) is connected to all core switches
    bool src_is_supernode = is_supernode(src);
    bool dest_is_supernode = is_supernode(dest);
    
    // Case 1: Regular node -> Supernode
    if (!src_is_supernode && dest_is_supernode && _tiers == 3) {
        // Path: Server -> ToR -> Agg -> Core -> Supernode
        // Generate paths through all possible core switches for ECMP
        uint32_t src_tor = HOST_POD_SWITCH(src);
        uint32_t src_pod = HOST_POD(src);
        
        for (uint32_t upper = MIN_POD_AGG_SWITCH(src_pod); upper <= MAX_POD_AGG_SWITCH(src_pod); upper++) {
            ...
            for (uint32_t l = 0; l < _radix_up[AGG_TIER]/_bundlesize[CORE_TIER]; l++) {
                ...
                for (uint32_t b1_up = 0; b1_up < _bundlesize[AGG_TIER]; b1_up++) {
                    for (uint32_t b2_up = 0; b2_up < _bundlesize[CORE_TIER]; b2_up++) {
                        ...
                        // Server -> ToR
                        routeout->push_back(queues_ns_nlp[src][src_tor][0]);
                        routeout->push_back(pipes_ns_nlp[src][src_tor][0]);
                        if (_qt==LOSSLESS_INPUT || _qt==LOSSLESS_INPUT_ECN)
                            routeout->push_back(queues_ns_nlp[src][src_tor][0]->getRemoteEndpoint());
                        
                        // ToR -> Agg
                        routeout->push_back(queues_nlp_nup[src_tor][upper][b1_up]);
                        routeout->push_back(pipes_nlp_nup[src_tor][upper][b1_up]);
                        if (_qt==LOSSLESS_INPUT || _qt==LOSSLESS_INPUT_ECN)
                            routeout->push_back(queues_nlp_nup[src_tor][upper][b1_up]->getRemoteEndpoint());
                        ...
                        // Core -> Supernode
                        routeout->push_back(queues_core_supernode[core]);
                        routeout->push_back(pipes_core_supernode[core]);
                        
                        if (reverse) {
                            routeback = new Route();
                            
                            // Supernode -> Core
                            routeback->push_back(queues_supernode_core[core]);
                            routeback->push_back(pipes_supernode_core[core]);
                            ...
                            // Agg -> ToR
                            routeback->push_back(queues_nup_nlp[upper][src_tor][b1_up]);
                            routeback->push_back(pipes_nup_nlp[upper][src_tor][b1_up]);
                            if (_qt==LOSSLESS_INPUT || _qt==LOSSLESS_INPUT_ECN)
                                routeback->push_back(queues_nup_nlp[upper][src_tor][b1_up]->getRemoteEndpoint());
                            
                            // ToR -> Server
                            routeback->push_back(queues_nlp_ns[src_tor][src][0]);
                            routeback->push_back(pipes_nlp_ns[src_tor][src][0]);
        ...
        cout << "pathcount (regular->supernode) " << paths->size() << endl;
        return paths;
    }
    
    // Case 2: Supernode -> Regular node
    if (src_is_supernode && !dest_is_supernode && _tiers == 3) {
        // Path: Supernode -> Core -> Agg -> ToR -> Server
        // Generate paths through all possible core switches for ECMP
        uint32_t dest_tor = HOST_POD_SWITCH(dest);
        uint32_t dest_pod = HOST_POD(dest);
        
        for (uint32_t core = 0; core < NCORE; core++) {
            uint32_t upper2 = MIN_POD_AGG_SWITCH(dest_pod) + core % _agg_switches_per_pod;
            ...
            for (uint32_t b1_down = 0; b1_down < _bundlesize[AGG_TIER]; b1_down++) {
                for (uint32_t b2_down = 0; b2_down < _bundlesize[CORE_TIER]; b2_down++) {
                    ...
                    // Supernode -> Core
                    routeout->push_back(queues_supernode_core[core]);
                    routeout->push_back(pipes_supernode_core[core]);
                    ...
                    // Agg -> ToR
                    routeout->push_back(queues_nup_nlp[upper2][dest_tor][b1_down]);
                    routeout->push_back(pipes_nup_nlp[upper2][dest_tor][b1_down]);
                    if (_qt==LOSSLESS_INPUT || _qt==LOSSLESS_INPUT_ECN)
                        routeout->push_back(queues_nup_nlp[upper2][dest_tor][b1_down]->getRemoteEndpoint());
                    
                    // ToR -> Server
                    routeout->push_back(queues_nlp_ns[dest_tor][dest][0]);
                    routeout->push_back(pipes_nlp_ns[dest_tor][dest][0]);
                    
                    if (reverse) {
                        routeback = new Route();
                        
                        // Server -> ToR
                        routeback->push_back(queues_ns_nlp[dest][dest_tor][0]);
                        routeback->push_back(pipes_ns_nlp[dest][dest_tor][0]);
                        if (_qt==LOSSLESS_INPUT || _qt==LOSSLESS_INPUT_ECN)
                            routeback->push_back(queues_ns_nlp[dest][dest_tor][0]->getRemoteEndpoint());
                        
                        // ToR -> Agg
                        routeback->push_back(queues_nlp_nup[dest_tor][upper2][b1_down]);
                        routeback->push_back(pipes_nlp_nup[dest_tor][upper2][b1_down]);
                        if (_qt==LOSSLESS_INPUT || _qt==LOSSLESS_INPUT_ECN)
                            routeback->push_back(queues_nlp_nup[dest_tor][upper2][b1_down]->getRemoteEndpoint());
                        ...
                        // Core -> Supernode
                        routeback->push_back(queues_core_supernode[core]);
                        routeback->push_back(pipes_core_supernode[core]);
        ...
        cout << "pathcount (supernode->regular) " << paths->size() << endl;
        return paths;
    }
    
    // Case 3: Supernode -> Supernode (shouldn't happen, but handle gracefully)
    if (src_is_supernode && dest_is_supernode) {
        cerr << "Warning: Attempted to create path from supernode to itself" << endl;
        return paths;  // Return empty paths
    }
