...
// Main simulation file using FatTreeTopology with supernode support
// Based on main_eqds.cpp but uses fat_tree_topology_inc.h
#include <sstream>
#include <string.h>
...
// SUPERNODE: Use the topology with supernode support
#include "../topology/fat_tree_topology_inc.h"
...
int main(int argc, char **argv) {
    ...
    // SUPERNODE: Create topology with supernode support
    FatTreeTopology* top;
    if (topo_file) {
        ...
        // SUPERNODE: If using default K-ary fat-tree, no_of_nodes should be K^3/4 + 1 to include supernode
        // The topology will automatically detect and create the supernode
    ...
    // SUPERNODE: Print supernode information if it exists
    if (tiers == 3 && top->no_of_servers() < no_of_nodes) {
        uint32_t supernode_id = top->get_supernode_id();
        cout << "SUPERNODE: Supernode detected with node ID = " << supernode_id << endl;
        cout << "SUPERNODE: Regular servers: 0 to " << (supernode_id - 1) << endl;
        cout << "SUPERNODE: Total nodes: " << no_of_nodes << " (including supernode)" << endl;
    }
    ...
    for (size_t c = 0; c < all_conns->size(); c++){
        ...
        // SUPERNODE: With virtual pod approach, supernode is treated as a regular node
        // No special-case handling needed - it's in its own pod with a ToR switch
        switch (route_strategy) {
        case ECMP_FIB:
        case ECMP_FIB_ECN:
        case REACTIVE_ECN:
            {
                // All nodes (including supernode) use standard routing
    ...
    // SUPERNODE: Log supernode information
    if (tiers == 3 && top->no_of_servers() < no_of_nodes) {
        logfile.write("# SUPERNODE: Supernode ID = " + ntoa(top->get_supernode_id()));
        logfile.write("# SUPERNODE: Regular servers = " + ntoa(top->no_of_servers()));
        logfile.write("# SUPERNODE: Total nodes = " + ntoa(no_of_nodes));
    }
